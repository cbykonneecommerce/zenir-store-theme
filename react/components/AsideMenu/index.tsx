...
const Menu = () => {
  ...
  const [category, setCategory] = React.useState<
    Array<{
      ...
      children: Array<{
        ...
        url: string;
        hasChildren: boolean;
        children: Array<{ name: string; url: string; hasChildren: boolean }>;
      }>;
    }>
  >([]);
  ...
  const handleMenuItems = (back: boolean) => {
    const items: any = document.getElementsByClassName(
  ...
  const showAllButtons = () => {
    const items: any = document.querySelectorAll(
      ".vtex-store-drawer-0-x-menuItem .vtex-button"
    );
    ...
    return (
      <ul className="list pa0 dn vtex-store-drawer-0-x-subMenu">
        <li className="vtex-store-drawer-0-x-submenuItem bb b--light-gray bt flex items-center justify-between">
          <MenuButton back />
          <a
            className="db pv5 mh5 w-100 no-underline c-on-base t-body vtex-store-drawer-0-x-submenuLink "
            href={menuItem.url}
            title={menuItem.name}
          >
            {menuItem.name}
